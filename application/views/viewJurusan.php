    ...
    <div class="row">
        <div class="col-md-8">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Grid Jurusan</h3>
                </div><!-- /.box-header -->
                <div class="box-body table-responsive">
                    <table id="tabeljurusan" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                            	<th></th>
                            	<th>No</th>
        ...
        <div class="col-md-4">
            <div class="box box-primary">
                ...
                <!-- form start -->
                <form role="form" id="formjurusan">
                    <input type="hidden" name="task" id="task" value="" />
                    <input type="hidden" name="idjurusan" id="idjurusan" value="" />
                    <div class="box-body">
                        <div class="form-group">
                            <label for="namajurusan">Nama Jurusan</label>
                            <input type="text" class="form-control" id="namajurusan" placeholder="" name="namajurusan">
                        </div>
                    </div><!-- /.box-body -->
...
$("#btnsimpan").hide();
$("#btnbatal").hide();
$("#formjurusan input,select,textarea").attr('disabled',true);

$("#btntambah").click(function(e) {
  e.preventDefault();
  $("#task").val('tambah');
  $("#formjurusan input,select,textarea").attr('disabled',false);
  $("#btnsimpan").show();
  $("#btnbatal").show();
  $(this).hide();
});
...
$("#formjurusan").submit(function(e) {
    ...
    // formData += '&task='+task+'&idjurusan='+idjurusan;
    console.info(data);
    $.ajax({
        url: global.baseUrl+'con_jurusan/crud_periode',
        type: 'POST',
        dataType: 'json',
        data: formData,
        cache: false,
...
var table = $('#tabeljurusan').DataTable({
          "order": [[ 3, "asc" ]],
          "columns": [
          ...
          serverSide: true,
          ajax: {
            url: "<?php echo base_url(); ?>con_jurusan/getData",
        	type: "POST",
            data: function (d) {
...
function delete_data(id) {
    $.post("<?php echo base_url(); ?>con_jurusan/crud_periode", {'idjurusan': id,'task':'hapus'}, function (response) {
        if(response.status){
            NotifikasiToast({
                type : 'success', // ini tipe notifikasi success,warning,info,error
                msg : response.message, //ini isi pesan
...
function initEvent() {
    ...
    $("[id^=editData]").click(function (e) {
        ...
        $("#idjurusan").val(dataedit[0]);
        $("#namajurusan").val(dataedit[3]);
